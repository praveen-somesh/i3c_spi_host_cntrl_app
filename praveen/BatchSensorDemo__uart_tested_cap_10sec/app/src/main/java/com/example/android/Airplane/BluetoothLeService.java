    ...
    private void broadcastUpdate(final String action,
        ...
        if(TX_CHAR_UUID.equals(characteristic.getUuid())) {
            ...
            if (data != null && data.length > 0) {
                    ...
                    if(status_read_done==1 ) {
                        ...
                        if(count_19%61!=0) {
                          ...
                          //  command_fea = command_fea +stringBuilder.toString();
                            ...
                            datafile.saveToSDCard("sensor_buffer_200_2.txt",command_fea, 1);
                ...
                if ((data[0] & 0x80) != 0 && data.length == 1) {
                    interrupt = 1;
                ...
                if ((data[0] == 0x40)  ){
                 ...
                 /*   int sc=1;
                    while (sc<data.length && data.length==19)
                    {
                        short value;
                        ////////////////x-co-ordinates////////////////
                        // LoadObjFile.accx= ((data[sc]& 0xff)|(data[sc+3]<<8) );
                        value= (short) ((data[sc]& 0xff)|(data[sc+3]<<8));

                        if(LoadObjFile.calib_status==1) {
                            LoadObjFile.accx = value;
                            // Log.e(TAG, String.format("debug....acce_x merged_[%d-%d]:%f ", sc, sc + 3, LoadObjFile.accx));
                        }
                        if(LoadObjFile.calib_status==0) {
                        ...
                        value= (short) ((data[sc]& 0xff)|(data[sc+3]<<8));
                        if(LoadObjFile.calib_status==1) {
                            LoadObjFile.gyrox = value;
                            //  Log.e(TAG, String.format("debug....gyro_x merged_[%d-%d]:%f ", sc, sc + 3, LoadObjFile.gyrox));
                        }
                        if(LoadObjFile.calib_status==0) {
                        ...
                        sc++;
                        value= (short) ((data[sc+3]& 0xff)|(data[sc]<<8));
                        if(LoadObjFile.calib_status==1) {
                            LoadObjFile.compx = value;
                            // Log.e(TAG, String.format("debug....compas_x merged_[%d-%d]:%f ", sc, sc + 3, LoadObjFile.compx));
                        }
                        if(LoadObjFile.calib_status==0) {
                        ...
                        value= (short) ((data[sc]& 0xff)|(data[sc+3]<<8));
                        if(LoadObjFile.calib_status==1) {
                            LoadObjFile.accy = value;
                            // Log.e(TAG, String.format("debug....acce_y merged_[%d-%d]:%f ", sc, sc + 3, LoadObjFile.accy));
                        }
                        if(LoadObjFile.calib_status==0) {
                        ...
                        value= (short) ((data[sc]& 0xff)|(data[sc+3]<<8));
                        if(LoadObjFile.calib_status==1) {
                            LoadObjFile.gyroy = value;
                            // Log.e(TAG, String.format("debug....gyro_y merged_[%d-%d]:%f ", sc, sc + 3, LoadObjFile.gyroy));
                        }
                        if(LoadObjFile.calib_status==0) {
                        ...
                        value= (short) ((data[sc+3]& 0xff)|(data[sc]<<8));
                        if(LoadObjFile.calib_status==1) {
                            LoadObjFile.compy = value;
                            //Log.e(TAG, String.format("debug....compas_y merged_[%d-%d]:%f ", sc, sc + 3, LoadObjFile.compy));
                        }
                        if(LoadObjFile.calib_status==0) {
                        ...
                        value= (short) ((data[sc]& 0xff)|(data[sc+3]<<8));
                        if(LoadObjFile.calib_status==1) {
                            LoadObjFile.accz = value;
                            // Log.e(TAG, String.format("debug....acce_z merged_[%d-%d]:%f ", sc, sc + 3, LoadObjFile.accz));
                        }
                        if(LoadObjFile.calib_status==0) {
                        ...
                        value= (short) ((data[sc]& 0xff)|(data[sc+3]<<8));
                        if(LoadObjFile.calib_status==1) {
                            LoadObjFile.gyroz = value;
                            // Log.e(TAG, String.format("debug....gyro_z merged_[%d-%d]:%f ", sc, sc + 3, LoadObjFile.gyroz));
                        }
                        if(LoadObjFile.calib_status==0) {
                        ...
                        value= (short) ((data[sc+3]& 0xff)|(data[sc]<<8));
                        if(LoadObjFile.calib_status==1) {
                            LoadObjFile.compz = value;
                            //Log.e(TAG, String.format("debug....compas_z merged_[%d-%d]:%f ", sc, sc + 3, LoadObjFile.compz));
                        }
                        if(LoadObjFile.calib_status==0) {
                        ...
                        sc+=4;
                        ////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
                        // if(LoadObjFile.calib_status==1)LoadObjFile. airplane();
                        if(LoadObjFile.calib_status==0)
                        {
                            AutoCalibration.do_calibration();
